            ...
            <div class="container">
               <div class="row">
                  <div class="col-xxl-4 offset-xxl-4">
                     <div class="section__title-wrapper mb-20 text-center">
                     ...
                     <div class="section__title-wrapper mb-20 col-md-2 text-end">
                       <?php
                         $tests = DB::table('tests')->select('mcq_test_id')->where('tests.teacher_id' , '=' , Auth::id())->get();
                        // print_r  ($tests);
                       foreach ($tests as $test) {
                      $mcq_test_id = json_decode($test->mcq_test_id);
                     

                        if(in_array($id , $mcq_test_id)){
                           ?>
                            <a href="{{url('MK_MCQ_test')}}" class="e-btn">Edit</a>
                           <?php
                        }
                       }
                        ?>
               ...
               @foreach($McqTestQuestion as $McqTestQuestions)
                    <div class="row">
                  <div class="col-xxl-10 offset-xxl-1 col-xl-10 offset-xl-1 col-lg-10 offset-lg-1">
                     <div class="events__item mb-10 hover__active">
                        <div class="events__item-inner d-sm-flex align-items-center justify-content-between white-bg">
                           ...
                           <?php    
                           // $arr =explode('.',$McqTestQuestions->question);
                           // $is_image_question = in_array(end($arr) , $extension);


                           // $arr =explode('.',$McqTestQuestions->answer1);
                           // $is_image_answer1 = in_array(end($arr) , $extension);
                           

                           // $arr =explode('.',$McqTestQuestions->answer2);
                           // $is_image_answer2 = in_array(end($arr) , $extension);


                           // $arr =explode('.',$McqTestQuestions->answer3);
                           // $is_image_answer3 = in_array(end($arr) , $extension);


                           // $arr =explode('.',$McqTestQuestions->answer4);
                           // $is_image_answer4 = in_array(end($arr) , $extension);
                        ?>

                        {{-- @if($is_image_question)
                           <img src="{{asset('mcq_img/'.$McqTestQuestions->question)}}" width="200" height="200">
                           @else 
                           <h3 class="events__title">Q{{$loop->iteration}}.{{$McqTestQuestions->question}}</h3>
                           @endif --}}

                           <h3 class="events__title">Q{{$loop->iteration}}.{{$McqTestQuestions->question}}</h3>
                           @if($McqTestQuestions->question_img)
                           <img src="{{asset($McqTestQuestions->question_img)}}" width="200" height="200">
                           @endif
							  ...
							  <br>
							   <div class="events__meta">
                                 ...
                                 <label htmlFor="rad1">{{$McqTestQuestions->answer1}}</label>
                                 @if($McqTestQuestions->answer1_img)
                                 <img src="{{asset($McqTestQuestions->answer1_img)}}" width="200" height="200">
                                 @endif
                                
								 <br>

								<input type="radio" name="group" value="new2" id="rad2">
                        <label htmlFor="rad2">{{$McqTestQuestions->answer2}}</label>
                        @if($McqTestQuestions->answer2_img)
                        <img src="{{asset($McqTestQuestions->answer2_img)}}" width="200" height="200">
                        @endif
                        ...
                        <label htmlFor="rad3">{{$McqTestQuestions->answer3}}</label>
                        @if($McqTestQuestions->answer3_img)
                        <img src="{{asset($McqTestQuestions->answer3_img)}}" width="200" height="200">
                        @endif
                        ...
                        <label htmlFor="rad4">{{$McqTestQuestions->answer4}}</label>
                        @if($McqTestQuestions->answer4_img)
                        <img src="{{asset($McqTestQuestions->answer4_img)}}" width="200" height="200">
                        @endif
                      
								
                              </div>
							  
							  <p style="color: #fff;">Answer : {{$McqTestQuestions->right_ans}}</p>
